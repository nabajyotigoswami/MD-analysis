...
def find_input_file():
    """Find the likely MM data file without requiring interactive input."""
    ...
    mm_candidates = [
        ...
        if os.path.basename(path).lower().startswith("mm-")
    ]
    if len(mm_candidates) == 1:
        return mm_candidates[0]
    if len(candidates) == 1:
        return candidates[0]
    ...
    raise ValueError(
        ...
        "python block_analysis-MM.py --input /full/path/to/your/file.xlsx\n\n"
        f"Files found:\n{names}"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Calculate block statistics from an MM analysis Excel or CSV file."
    )
    parser.add_argument(
        "-i", "--input", dest="input_file",
        help="Input .xlsx, .xls, or .csv file. Automatically finds an MM file when omitted."
    )
    parser.add_argument(
...
def main():
    ...
    if args.block_size <= 0 or args.total_time <= 0:
        raise ValueError("Block size, total time, and time interval must be greater than zero.")
    ...
    # MM input time values are already recorded in ns: 0, 0.5, 1, 1.5, ...
    df[time_col] = pd.to_numeric(df[time_col], errors="coerce")
    df = df.dropna(subset=[time_col]).copy()
    system_columns = list(df.columns[1:])

    print("\nTime column:", time_col)
    print("Time values are interpreted directly in ns (0.5 ns intervals).")
    print("\nSystems detected:")
