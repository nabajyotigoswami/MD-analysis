...
def find_input_file():
    """Find the likely SA data file without requiring interactive input."""
    ...
    sa_candidates = [
        ...
        if os.path.basename(path).lower().startswith("sa-wt-")
    ]
    if len(sa_candidates) == 1:
        return sa_candidates[0]
    if len(candidates) == 1:
        return candidates[0]
    ...
    raise ValueError(
        ...
        "python block_analysis-SA.py --input /full/path/to/your/file.xlsx\n\n"
        f"Files found:\n{names}"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Calculate block statistics from an SA analysis Excel or CSV file."
    )
    parser.add_argument(
        "-i", "--input", dest="input_file",
        help="Input .xlsx, .xls, or .csv file. Automatically finds an SA-WT- file when omitted."
    )
    parser.add_argument(
...
def main():
    ...
    # SA input time values are interpreted directly as ns.
