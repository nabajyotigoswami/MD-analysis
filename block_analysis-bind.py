...
def find_input_file():
    """Find the likely binding-energy data file without interactive input."""
    ...
    bind_candidates = [
        ...
        if os.path.basename(path).startswith(INPUT_PREFIX)
    ]
    if len(bind_candidates) == 1:
        return bind_candidates[0]
    if len(candidates) == 1:
        return candidates[0]
    ...
    raise ValueError(
        ...
        "python block_analysis-bind.py --input /full/path/to/your/file.xlsx\n\n"
        f"Files found:\n{names}"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Calculate block statistics from a binding-energy Excel or CSV file."
    )
    parser.add_argument(
        "-i", "--input", dest="input_file",
        help="Input .xlsx, .xls, or .csv file. Automatically finds a ΔGbind file when omitted."
    )
    parser.add_argument(
...
def main():
    ...
    # Binding-energy input time values are interpreted directly as ns.
    df[time_col] = pd.to_numeric(df[time_col], errors="coerce")
    df = df.dropna(subset=[time_col]).copy()
    system_columns = list(df.columns[1:])

    print("\nTime column:", time_col)
    print("Time values are interpreted directly in ns.")
    print("\nSystems detected:")
