...
def find_input_file():
    """Find the likely SASA data file without requiring interactive input."""
    ...
    sasa_candidates = [
        ...
        if "sasa" in os.path.basename(path).lower()
    ]
    if len(sasa_candidates) == 1:
        return sasa_candidates[0]
    if len(candidates) == 1:
        return candidates[0]
    ...
    raise ValueError(
        ...
        "python block_analysis-SASA.py --input /full/path/to/your/file.xlsx\n\n"
        f"Files found:\n{names}"
    )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Input .xlsx, .xls, or .csv file. Automatically finds an SASA file when omitted."
    )
    parser.add_argument(
