...
def find_input_file():
    """Find the likely PB data file without requiring interactive input."""
    ...
    pb_candidates = [
        ...
        if os.path.basename(path).lower().startswith("pb-")
    ]
    if len(pb_candidates) == 1:
        return pb_candidates[0]
    if len(candidates) == 1:
        return candidates[0]
    ...
    raise ValueError(
        ...
        "python block_analysis-PB.py --input /full/path/to/your/file.xlsx\n\n"
        f"Files found:\n{names}"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Calculate block statistics from a PB analysis Excel or CSV file."
    )
    parser.add_argument(
        "-i", "--input", dest="input_file",
        help="Input .xlsx, .xls, or .csv file. Automatically finds a PB file when omitted."
    )
    parser.add_argument(
...
def main():
    ...
    # PB input time values are already recorded in ns: 0, 0.5, 1, 1.5, ...
